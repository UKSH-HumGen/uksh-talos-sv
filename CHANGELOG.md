...
## [1.0.0-uksh] - 2026-05-11

First public release of the UKSH Talos-SV fork. Based on
[populationgenomics/talos](https://github.com/populationgenomics/talos) v8.2.0.

### Added

* `CategorizeSVs.py`: GFF3-based gene/exon annotation for structural variants.
* Multi-sample SV merging that respects per-sample genotype calls (so a CNV is
  only assigned to samples that actually carry it, not to every sample in a
  joint-called VCF).
* MOI validation extended to structural variants in `utils.py` /
  `ValidateMOI.py`.
* DRAGEN CNV display fix in the HTML report.
* `MergeVcfsWithBcftools` modification that preserves SV coordinates instead
  of collapsing them to representative breakpoints.
* Synthetic example pedigree (`nextflow/inputs/pedigree_synthetic_example.ped`)
  and example case sheet (`nextflow/inputs/Cases_example.txt`) for users to
  exercise the workflow without supplying their own data.

### Changed

* Renamed package to `uksh-talos-sv` (the importable Python package keeps the
  upstream name `talos` for drop-in compatibility).
* `cpg-utils` moved from a hard runtime dependency to an optional extra under
  `[cpg]`. Modules that need `init_batch` fall back to a no-op stub when the
  package is not installed.
* Docker image renamed/tagged `uksh-talos-sv:1.0.0`.

### Experimental (not integrated into the main workflow)

* `MergeStructuralVariants` Nextflow module — moved to `experimental/`.
* `RescueCompoundHet` Nextflow module and `rescue_compound_het.py` — moved to
  `experimental/`.

### Removed (from upstream)

* CPG-specific `cpg_internal_scripts/` subpackage (cpg-flow stages,
  metamist/seqr integration, `BuildReportIndexPage`, `MinimiseOutputForSeqr`,
  internal GCP build scripts).
* Upstream `CPG_Dockerfile` (the standalone `Dockerfile` at the repo root
  is now the canonical image definition).
* Internal `DENOVO_NONPANEL_VARIANT_MODULE.md` design doc (moved to the
  project's private notes directory).

---

## Upstream Talos changelog (preserved below for provenance)
