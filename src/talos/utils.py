...
def gather_gene_dict_from_contig(
    ...
    if sv_source:
        ...
        for variant in sv_source(contig):
            ...
            # update the variant count
            sv_from_separate_vcf += 1

            # update the gene index dictionary
            contig_dict[structural_variant.info['gene_id']].append(structural_variant)

        logger.info(f'Contig {contig} contained {sv_from_separate_vcf} SVs from separate VCF')

    if structural_variants > 0:
        logger.info(f'Contig {contig} contained {structural_variants} SVs from merged VCF')
