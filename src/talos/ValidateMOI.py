...
def filter_structural_variants(results_holder: ResultData) -> None:
    """
    Comprehensive structural variant filtering based on config settings.

    Filters SVs based on:
    1. SV type (allowed_sv_types from config)
    2. Exon overlap requirements (require_exon_overlap, filter_intronic_svs)
    3. Frequency thresholds (AF filters from ValidateMOI config)
    4. Size requirements (min_sv_size, max_sv_size)
    5. Quality filters (require_pass_only)

    This filtering is controlled by config: [StructuralVariantFiltering]
    ...
    callset_sv_max_af = config_retrieve(['ValidateMOI', 'callset_sv_max_af'], 0.01)
    min_callset_ac = config_retrieve(['ValidateMOI', 'min_callset_ac_to_filter'], 10)

    # Dominant-specific thresholds
    dominant_gnomad_sv_max_af = config_retrieve(['ValidateMOI', 'dominant_gnomad_sv_max_af'], 0.01)
    dominant_callset_sv_max_af = config_retrieve(['ValidateMOI', 'dominant_callset_sv_max_af'], 0.01)
    ...
    for sample_id, participant_data in results_holder.results.items():
        ...
        kept_variants = []
        for variant in participant_data.variants:
            # Check if this is a structural variant
            coords = variant.var_data.coordinates
            is_sv = coords.ref.startswith('<') and coords.ref.endswith('>')
            ...
            # Get SV attributes
            sv_type = variant.var_data.info.get('svtype', '').upper()
            svlen_raw = variant.var_data.info.get('svlen', 0)
            # Handle svlen as tuple (multi-allelic) or int
            sv_size = abs(svlen_raw[0] if isinstance(svlen_raw, tuple) else svlen_raw) if svlen_raw else 0
            exon_overlap_bp = variant.var_data.info.get('exon_overlap_bp', 0)
            gnomad_af = variant.var_data.info.get('gnomad_sv_AF', 0.0) or 0.0
            callset_af = variant.var_data.info.get('AF', 0.0) or 0.0
            callset_ac = variant.var_data.info.get('AC', 0) or 0

            # Check if compound het or dominant
            is_comp_het = 'Comp-Het' in variant.reasons
            is_dominant = 'Autosomal Dominant' in variant.reasons or 'X-Linked Dominant' in variant.reasons

            # Filter 1: SV type
            if sv_type not in allowed_sv_types:
                filtered_by_type += 1
                continue

            # Filter 2: Size requirements
            if min_sv_size > 0 and sv_size < min_sv_size:
                filtered_by_size += 1
                continue
            if max_sv_size > 0 and sv_size > max_sv_size:
                filtered_by_size += 1
                continue

            # Filter 3: Exon overlap (strict requirement)
            if require_exon_overlap and exon_overlap_bp < min_exon_overlap_bp:
                filtered_by_exon_overlap += 1
                continue

            # Filter 4: Intronic filtering (less strict - allows comp-hets)
            if filter_intronic_svs and exon_overlap_bp < min_exon_overlap_bp:
                # For intronic SVs, only keep if they're part of compound het AND pass additional filters
                if is_comp_het:
                    # Check intronic comp-het size
                    if min_intronic_comp_het_size > 0 and sv_size < min_intronic_comp_het_size:
                        filtered_by_intronic += 1
                        continue
                    # Check intronic comp-het type
                    if sv_type not in allowed_intronic_comp_het_types:
                        filtered_by_intronic += 1
                        continue
                else:
                    # Intronic and not comp-het - filter it out
                    filtered_by_intronic += 1
                    continue

            # Filter 5: Frequency filtering
            # Choose appropriate threshold based on MOI
            af_threshold = dominant_gnomad_sv_max_af if is_dominant else gnomad_sv_max_af
            callset_af_threshold = dominant_callset_sv_max_af if is_dominant else callset_sv_max_af
            ...
            # Apply callset AF filter (only if AC is above threshold)
            if callset_ac >= min_callset_ac and callset_af > callset_af_threshold:
                filtered_by_af += 1
                continue
...
def filter_results_to_panels(
    ...
    =====================================
    Variants are normally filtered unless their gene matches the participant's assigned panels.
    However, de novo variants with pathogenicity support can bypass this filter.

    Rationale:
    - De novo variants often reveal unexpected diagnoses that don't match initial phenotype
    - This is especially true for syndromic conditions where the full phenotype may not be apparent
    - Requiring pathogenicity support reduces false positives from poor inheritance calls

    Bypass Requirements:
    1. Variant must have 'De Novo' category (indicating de novo inheritance)
    2. Variant must have pathogenicity support from at least one of:
       - 'ClinVar P/LP' (ClinVar Pathogenic/Likely Pathogenic)
       - 'AlphaMissense' (AlphaMissense likely_pathogenic prediction)
       - 'High Impact' (High impact consequence: stop gain, frameshift, etc.)

    When bypass occurs:
    - Variant is included in results despite panel mismatch
    - Logged for audit trail with gene, sample, and pathogenicity categories
    - Standard panel assignment still occurs (may be empty sets)
    ...
    for each_event in result_list:
        ...
        if denovo_bypass and not (forced_panels_for_this_gene or natural_matches_for_this_gene or (default_panel in gene_panels)):
            ...
            logger.info(
                f'De novo variant {each_event.var_data.coordinates.string_format} '
                f'for sample {each_event.sample} in gene {gene_symbol} bypassed panel filtering '
                f'(pathogenicity categories: {pathogenicity_cats})'
            )
...
def main(
    ...
    # comprehensive structural variant filtering (type, exon overlap, AF, size)
    filter_structural_variants(results_model)
