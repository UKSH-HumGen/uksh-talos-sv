...
def remove_phenotype_required_variants(result_object: ResultData):
    ...
    for participant in result_object.results.values():
        ...
        for variant in participant.variants:
            ...
            # skip this variant
            # IMPORTANT: De novo variants should NEVER be filtered by phenotype match
            non_denovo_categories = {cat for cat in variant.categories if cat not in ['De Novo', 'denovo', '4']}
            if (
                (not matched_variant)
                and (len(variant.support_vars) == 0)
                and non_denovo_categories  # Must have some non-de-novo categories to filter
                and (all(cat in cats_require_pheno_match for cat in non_denovo_categories))
            ):
                continue
