...
def filter_on_quality_flags(mt: hl.MatrixTable) -> hl.MatrixTable:
    """
    Filter MT to rows with 0 quality filters or low QD scores that we want to keep.
    Note: in Hail, PASS is represented as an empty set.

    This is overridden with Clinvar Pathogenic or QD > 0.1.
...
def annotate_category_alphamissense(mt: hl.MatrixTable) -> hl.MatrixTable:
    ...
    # First, annotate transcript_consequences with am_class if it's missing but am_pathogenicity is present
    mt = mt.annotate_rows(
